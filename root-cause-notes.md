...
- First remote exact random-fixed attempt (`1149517`) failed before it could test model performance: current fail-closed `LoadFrames._remap_gt_to_selected_axis()` rejected a historical random-fixed case where one GT segment collapsed after selected-axis remap (`video_validation_0000932`, `dropped=1`, `kept=13`, `input=14`). This is a data-contract mismatch between the new safety guard and the historical 0410 reproduction, not evidence that pure dense ActionFormer collapsed.
- The exact 0410 random-fixed config now explicitly opts into `allow_drop_selected_axis_gt=True` with `legacy_selected_axis_gt_drop_diagnostic=True` on all three `LoadFrames` steps. This preserves the historical baseline reproduction while keeping the fail-closed guard active for new non-legacy routes.
- Interpretation gate:
