...
QUEUE_LOG="$LOG_DIR/irregular_repair_queue_server3.log"
GPU_ID="${GPU_ID:-0}"
...
CONFIGS=(
  "configs/adatad/thumos/input_random_fixed_50pct_irregular_actionformer_headv3_oabs_full_x_legacysingleton.py"
  "configs/adatad/thumos/input_random_fixed_50pct_irregular_actionformer_headv3_oabs_full_x_fixsingleton_repaired.py"
)
...
echo "$(date '+%F %T') irregular repair queue server3 start" | tee -a "$QUEUE_LOG"
wait_for_existing_jobs
...
echo "$(date '+%F %T') irregular repair queue server3 completed" | tee -a "$QUEUE_LOG"
