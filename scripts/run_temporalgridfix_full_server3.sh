...
QUEUE_LOG="$LOG_DIR/temporalgridfix_full_server3.log"
GPU_ID="${GPU_ID:-0}"
...
run_exp() {
  ...
  cd "$ROOT_DIR"
  CUDA_VISIBLE_DEVICES="$GPU_ID" "$TORCHRUN" --nproc_per_node=1 --master_port="$master_port" tools/train.py "$config_path" 2>&1 | tee "$log_file"
  echo "$(date '+%F %T') finished ${name}" | tee -a "$QUEUE_LOG"
}

CFG_PATH="configs/adatad/thumos/input_random_fixed_50pct_irregular_actionformer_headv3_x_temporalgridfix.py"
EXP_NAME="input_random_fixed_50pct_irregular_actionformer_headv3_x_temporalgridfix"

echo "$(date '+%F %T') temporalgridfix full server3 start" | tee -a "$QUEUE_LOG"
check_cfg "$CFG_PATH"
wait_for_gpu
run_exp "$CFG_PATH" "$EXP_NAME" 29835
echo "$(date '+%F %T') temporalgridfix full server3 done" | tee -a "$QUEUE_LOG"
