...
QUEUE_LOG="$LOG_DIR/temporalgridfix_diag_server1.log"
GPU_ID="${GPU_ID:-0}"
...
CFG_PATH="configs/adatad/thumos/input_random_fixed_50pct_irregular_actionformer_temporalgridfix_diag_epoch1.py"
EXP_NAME="input_random_fixed_50pct_irregular_actionformer_temporalgridfix_diag_epoch1"

echo "$(date '+%F %T') temporalgridfix diag server1 start" | tee -a "$QUEUE_LOG"
check_cfg "$CFG_PATH"
wait_for_gpu
run_exp "$CFG_PATH" "$EXP_NAME" 29831
echo "$(date '+%F %T') temporalgridfix diag server1 done" | tee -a "$QUEUE_LOG"
