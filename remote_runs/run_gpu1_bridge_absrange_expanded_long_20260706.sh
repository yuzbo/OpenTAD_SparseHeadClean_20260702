...
LOG_DIR="$ROOT/logs/gpu1_bridge_absrange_expanded_long"
RUN_TAG="${RUN_TAG:-gpu1_bridge_absrange_expanded_long_$(date +%Y%m%d_%H%M%S)}"
CHAIN_LOG="$LOG_DIR/${RUN_TAG}.log"
TRAIN_LOG="$LOG_DIR/${RUN_TAG}_train.log"
CFG="configs/adatad/thumos/input_random_fixed_50pct_adapter_irregular_bridge_hard_linear_absrange_expanded_n16r4.py"
EXP_ID="${EXP_ID:-1}"
PORT="${PORT:-32326}"
...
log_msg "absrange_expanded long-train start on $(hostname) cfg=$CFG exp_id=$EXP_ID"
log_msg "root=$ROOT"
...
print("center_radius_scale", head.center_radius_scale, "reg_denom_mode", head.reg_denom_mode)
print("range_mode", prior.range_mode, "regression_range", prior.regression_range)
print("decode_scale_mode", prior.decode_scale_mode, "radius_scale_mode", prior.radius_scale_mode)
print("audit_reference", "pos_by_level=[17,30,44,45,22,4]", "gt_coverage=38/38")
print("compare_to", "openrange_final_42.44", "HeadV3_fixed_40.20", "equal_interval_reference_approx_65")
print("work_dir", work_dir)
PY

log_msg "py_compile preflight"
python -m py_compile tools/train.py opentad/models/dense_heads/irregular_actionformer_bridge_head.py opentad/models/dense_heads/prior_generator/irregular_point_generator.py
